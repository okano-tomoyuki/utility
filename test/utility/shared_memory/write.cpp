...
int main()
{
    ...
    // 先に起動するプロセスでのみ実行
    // 前回終了時にロックがかかった状態でプロセスを停止した場合を考慮したデッドロック対策
    // shared_memory.flush();
    ...
    while(!Signal::end_sign)
    {
        ...
        std::strcpy(sample_data.update_time, get_iso_format().c_str());
        sample_data.d_data += 1.1; 
        ...
        if(shared_memory.try_write(sample_data, 30))
        {
            std::cout << "[Write]: OK" << std::endl;
        }
        else
        {
            std::cout << "[Write]: NG" << std::endl;
        }
